...
public class PlayerInputs3 : MonoBehaviour, CustomInputs.ISpawnControllerActions
{
    ...
    public void OnKeyBoard(InputAction.CallbackContext context)
    {
        ...
        if (InputVector.x > 0)
        {
            if (dungeonGenerator.shitMap.GetTile(currentPosition + Vector3Int.right) == dungeonGenerator.tileBase[0])
            {
        ...
        {
            if (dungeonGenerator.shitMap.GetTile(currentPosition + Vector3Int.left) == dungeonGenerator.tileBase[0])
            {
